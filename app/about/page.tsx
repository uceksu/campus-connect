      ...
      <section className="mx-auto max-w-4xl px-6 py-14 sm:px-10 lg:px-16 lg:py-20 space-y-10">
        ...
        <div className="rounded-3xl border border-white/30 bg-white shadow-2xl overflow-hidden">
          ...
          <div className="p-8 sm:p-10 space-y-10">
            ...
            {/* Campus Info */}
            <div className="rounded-2xl border border-slate-100 bg-slate-50/50 p-6 sm:p-8 space-y-3">
              <h3 className="flex items-center gap-2 text-lg font-black text-[#071333] uppercase tracking-tight">
                <MapPin size={20} className="text-[#456be5]" /> Campus Information
              </h3>
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
            ...
            <div className="space-y-6">
              <h3 className="flex items-center gap-2 text-lg font-black text-[#071333] uppercase tracking-tight border-b border-slate-100 pb-3">
                <Users size={20} className="text-[#456be5]" /> Governing Body
              </h3>
              <div className="grid gap-6 sm:grid-cols-3">
                {governingBody.map((member) => (
                  <div
                    key={member.role}
                    className="rounded-2xl border border-slate-100 bg-slate-50/30 p-5 flex flex-col justify-between hover:border-blue-200 transition-colors shadow-sm"
                  >
                    <div>
                      <span className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider bg-blue-50 text-blue-600 border border-blue-100 mb-4">
                        {member.role}
                      </span>
                      <h4 className="text-base font-black text-[#071333] leading-snug">
                        {member.name}
                      </h4>
                    </div>
                    <div className="mt-4 text-xs text-slate-500 font-semibold space-y-0.5">
                      <p>{member.designation}</p>
                      <p className="text-slate-400 font-medium">{member.affiliation}</p>
                    </div>
                  </div>
                ))}
              </div>
