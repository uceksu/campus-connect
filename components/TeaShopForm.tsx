...
import { createTeaShop, updateTeaShop } from "@/lib/actions/teashop";
...
type Props = {
  submitLabel: string;
  teaShopId?: string;
  initialData?: Partial<TeaShop>;
};

export default function TeaShopForm({ submitLabel, teaShopId, initialData }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("saving");
      if (teaShopId) {
        await updateTeaShop(teaShopId, { ...values, imageUrl });
      } else {
        await createTeaShop({ ...values, imageUrl });
      }
      router.push("/admin/teashops");
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleChange}
            placeholder="About this tea shop..."
            rows={4}
            required
      ...
      <div className="flex gap-3">
        <button type="button" onClick={() => router.push("/admin/teashops")} className="px-6 py-2.5 rounded-xl border border-white/10 text-slate-300 hover:bg-white/5 transition-colors text-sm font-medium">
          Cancel
        </button>
