...
import { deleteAcademicNote } from "@/lib/actions/academicNote";
import { Trash2, Loader2 } from "lucide-react";

export default function DeleteAcademicNoteButton({ id }: { id: string }) {
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    const confirmed = window.confirm("Are you sure you want to delete this academic note?");
    if (!confirmed) return;
